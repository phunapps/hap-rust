...
- [ ] Item 5 — Onvis validation + publish. **Commissioning DONE + hardware-validated**: `hap-ble` 0.7.0 `thread_provision` moved a real Onvis SMS2 onto `OpenThread-89d7` over BLE (SRP-registered `_hap._udp` on the mesh), and `hap-thread` reaches it over Thread (CoAP round-trip). **Open:** authenticated read over Thread needs a cross-transport pairing (commissioning used an ephemeral controller → re-commission persisting the pairing, then Pair Verify over Thread); then `0x09` tree decode (deferred) and the **user-gated publish**. BLE runs on the Pi (macOS-26 `objc2` blocker on the Mac).
