...
- [x] Item 5 — Onvis validation + publish. **PUBLISHED** (hap-crypto 1.4.0, hap-ble 0.7.0, hap-controller 3.0.3, hap-thread 0.1.0, hap-thread-dut 0.1.0; tags pushed). **Full chain hardware-validated on the real SMS2:** commission over BLE (`hap-ble` 0.7.0 `thread_provision`) → Pair Verify over Thread → **`0x09` database read (2737 bytes; decodes to the full SMS2 database, MotionDetected iid 3074)** → subscribe → **live MotionDetected events over the radio** (`value=[1]` detect + `value=[0]` clear). Transport bugs found+fixed along the way: Block2 continuation framing (empty payload + fresh token) and the decisive `RECV_BUF` 1500→16 KiB (the Onvis returns `0x09` as one ~2.7 KB IPv6-fragmented datagram — truncation was the AEAD failure). Events built (`recv_event` server + `open_event` + `decode_events` + `subscribe`/`next_event`). BLE runs on the Pi (macOS-26 `objc2` blocker on the Mac).

## Post-roadmap follow-ups (all shipped)
- [x] **`0x09` → typed `hap-model` tree decode** — `hap_thread::decode_database` + `ThreadAccessory::read_database()`, cross-verified against aiohomekit on the committed real SMS2 body. Published **hap-thread 0.2.0**.
- [x] **Events `Stream` API** — `ThreadAccessory::watch_events() -> Stream<(iid, value)>` over a background task. Published **hap-thread 0.3.0**.
- [x] **Single-socket connection-actor demux** — one reader task owns the socket and demuxes request responses from accessory event PUTs, so reads/writes and `watch_events()` run **concurrently** on one accessory. Published **hap-thread 0.4.0**.
- [x] **End-to-end events coverage in CI** — `hap-thread-dut` now implements event **subscribe/unsubscribe + push** (`push_event`), and `crates/hap-thread-dut/tests/events.rs` drives the real controller through subscribe → push → decrypt and proves a concurrent read + event-watch on one accessory (regression guard for the demux) — all over UDP, no hardware.
