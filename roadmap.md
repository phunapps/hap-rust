...
- [ ] Item 5 — Onvis validation + publish. **Commissioning + authenticated Thread session DONE + hardware-validated**: `hap-ble` 0.7.0 `thread_provision` moved a real Onvis SMS2 onto `OpenThread-89d7` over BLE, and the `onvis_thread` combined flow reused that pairing to **Pair Verify over Thread** (encrypted session over the radio). **Bug found on hardware:** the `0x09` database read returns `Crypto(Aead)` — the CoAP `0x09` encrypted-read/Block2 framing (deferred in MT-1) needs an aiohomekit capture to finish. **Open:** capture + fix the `0x09` read, decode the tree, read sensors, then the **user-gated publish**. BLE runs on the Pi (macOS-26 `objc2` blocker on the Mac).
